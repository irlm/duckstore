...
public sealed class EtlService(WarehouseBuilder builder, IHostApplicationLifetime lifetime, ILogger<EtlService> log)
{
    ...
    public EtlRunSnapshot Start(string trigger, EtlMode mode = EtlMode.Full)
    {
        var run = Begin(trigger, mode);
    ...
    public Task RunAsync(string trigger, CancellationToken ct, EtlMode mode = EtlMode.Full) => ExecuteAsync(Begin(trigger, mode), ct);

    private Run Begin(string trigger, EtlMode mode)
    {
        lock (_gate)
        {
            ...
            _run = new Run(trigger, mode, WarehouseBuilder.ExpectedSteps(mode));
            return _run;
        }
    }
    ...
    private async Task ExecuteAsync(Run run, CancellationToken ct)
    {
        try
        {
            ...
            var result = await builder.BuildAsync(run.Mode, progress, ct);
    ...
    private sealed class Run(string trigger, EtlMode mode, int expectedSteps)
    {
        public EtlMode Mode { get; } = mode;
        ...
        public EtlRunSnapshot Snapshot() =>
            new(Id, trigger, StartedAt, FinishedAt, FinishedAt is null, Steps.ToList(), expectedSteps, Result, Error,
                Mode == EtlMode.Full ? "full" : "incremental");
    }
...
// (Etl:RunOnStartup, handy for the first `docker compose up`), run a full ETL
// every Etl:ScheduleMinutes and an incremental one every Etl:IncrementalMinutes
// (0 = off). A common setup: incremental every few minutes, full once a night.
// In production the schedule is often a cron job running
// `dotnet DuckStore.Analytics.dll etl [--incremental]` instead.
public sealed class EtlSchedule(EtlService etl, DuckStore.Analytics.Warehouse.WarehouseSettings settings, IConfiguration config, ILogger<EtlSchedule> log) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await Task.WhenAll(
            ScheduleAsync(config.GetValue("Etl:ScheduleMinutes", 0), EtlMode.Full, stoppingToken),
            ScheduleAsync(config.GetValue("Etl:IncrementalMinutes", 0), EtlMode.Incremental, stoppingToken));
    }

    private async Task ScheduleAsync(int minutes, EtlMode mode, CancellationToken stoppingToken)
    {
        if (minutes <= 0)
        {
            return;
        }
        log.LogInformation("{Mode} ETL scheduled every {Minutes} minutes", mode, minutes);
        using var timer = new PeriodicTimer(TimeSpan.FromMinutes(minutes));
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            try
            {
                await etl.RunAsync("schedule", stoppingToken, mode);
            }
            catch (EtlAlreadyRunningException)
            {
                log.LogInformation("Scheduled {Mode} ETL skipped: a run is already in progress", mode);
            }
            catch (Exception e) when (e is not OperationCanceledException)
            {
                log.LogError(e, "Scheduled {Mode} ETL failed", mode);
            }
        }
    }
}
